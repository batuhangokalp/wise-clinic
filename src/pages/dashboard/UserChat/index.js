...
import { fetchMessagesByConversationId, setChatMessagesPage } from "../../../redux/chat/actions";
import { throttle } from "lodash"

//actions
import { openUserSidebar, setChatMessages, setConversations } from "../../../redux/actions";
...
function UserChat(props) {
    ...
    useEffect(() => {
        if (socket) {
            ...
            const handleNewMessage = (message) => {
                ...
                let conversation = props.conversations.find(conversation => conversation?.phone_number === messageSender);
                if (conversation) {
                    conversation.last_message = message?.message ?? t('Media Message');
                    conversation.unRead = conversation.unRead && conversation.unRead >= 0 ? conversation.unRead + 1 : 1;
                    conversation.updated_at = new Date();

                    let currentConversations = props.conversations.filter(conversation => conversation?.phone_number !== messageSender);
    ...
    }, [socket, props.activeConversationId, chatMessages]);
    ...
    const addMessage = async (message, type) => {
        ...
        await dispatch(setChatMessages([...chatMessages, messageObj]))
    ...
    const chatMessagesLimit = useSelector(state => state.Chat?.chatMessagesLimit);
        ...
        if (ref.current) {
            ...
            if (scrollTop === 0 && !loadingOlderMessages) {
                ...
                setTimeout(() => {
                    ...
                    dispatch(fetchMessagesByConversationId(props.activeConversation, chatMessagesLimit, nextPage, true))
                        .then(() => {
                        ...
                        <ul className="list-unstyled mb-0">
                            {
                                chatMessages.map((chat, key) =>
                                    chat?.isToday && chat?.isToday === true ?
                                        ...
                                        (chat?.isGroup === true) ?
                                            <li key={key} className={chat?.sender_type === "user" ? "right" : ""}>
                                                            ...
                                                            <div className="chat-user-img align-self-center me-3">
                                                                <div className="avatar-xs">
                                                                    <span className="avatar-title rounded-circle bg-primary-subtle text-primary">
                                                                        {props.activeConversation?.contact_name?.charAt(0) ?? "A"}
                                                                    </span>
                                                                </div>
                                                            </div> :
                                                    ...
                                                    <div className="user-chat-content">
                                                        <div className="ctext-wrap">
                                                            <div className="ctext-wrap-content">
                                                                ...
                                                                {
                                                                    !chat?.isTyping && 
                                                                    <p className="chat-time mb-0"><i className="ri-time-line align-middle"></i> <span className="align-middle">{chat?.time}</span></p>
                                                                }
                                                            </div>
                                                            {
                                                                !chat?.isTyping &&
                                                        ...
                                                        {
                                                            <div className="conversation-name">{chat?.sender_type === "user" ? props.activeConversation?.contact_name : chat?.contact_name}</div>
                                                        }
                                                    </div>
                                                </div>
                                            </li>
                                                ...
                                                <div className="conversation-list">
                                                    {
                                                        ...
                                                        chatMessages[key + 1] ? chatMessages[key].sender_type === chatMessages[key + 1].sender_type ?
                                                            ...
                                                            <div className="chat-avatar">
                                                                {chat?.sender_type === "user" ?
                                                                    <div className="chat-user-img align-self-center me-3">
                                                                        <div className="avatar-xs">
                                                                            <span className="avatar-title rounded-circle bg-primary-subtle text-primary">
                                                                                {props.user?.name?.charAt(0) ?? "A"} {/* User Name Initials */}
                                                                            </span>
                                                                        </div>
                                                                    </div> :
                                                                    chat?.profilePicture === null ?
                                                                        ...
                                                                        : <div className="chat-user-img align-self-center me-3">
                                                                            <div className="avatar-xs">
                                                                                <span className="avatar-title rounded-circle bg-primary-subtle text-primary">
                                                                                    {props.activeConversation?.contact_name?.charAt(0) ?? "A"} {/*other user */}
                                                                                </span>
                                                                            </div>
                                                                        </div>
                                                                }
                                                            ...
                                                            : <div className="chat-avatar">
                                                                {chat?.sender_type === "user" ?
                                                                    <div className="chat-user-img align-self-center me-3">
                                                                        <div className="avatar-xs">
                                                                            <span className="avatar-title rounded-circle bg-primary-subtle text-primary">
                                                                                {props.user?.name?.charAt(0) ?? "A"} {/* User Name Initials last me*/}
                                                                            </span>
                                                                        </div>
                                                                    </div> :
                                                                    chat?.profilePicture === null ?
                                                    ...
                                                    <div className="user-chat-content">
                                                        <div className="ctext-wrap mb-3">
                                                            ...
                                                            </div>
                                                            {
                                                                ...
                                                                <>
                                                                    <br/>
                                                                <p className="chat-time mb-0"><i className="ri-time-line align-middle"></i> <span className="align-middle">{showChatMessageTime(chat?.created_at)}</span></p>
                                                                </>
                                                                }
                                                            {
                                                                !chat?.isTyping &&
                                                                <UncontrolledDropdown className="align-self-start ms-1">
                                                                    ...
                                                                    <DropdownMenu>
                                                                        {chat?.file_type_id && <DropdownItem onClick={() => downloadFile(chat?.file_path)}>{t('Download')} <i className="ri-file-download-line float-end text-muted"></i></DropdownItem>}
                                                        ...
                                                        {
                                                            chatMessages[key + 1] ?
                                                                chatMessages[key].sender_type === chatMessages[key + 1].sender_type ? null :

                                                                    <div className="conversation-name">{chat?.sender_type === "user" ?

                                                                        props.user?.name : chat?.contact_name}</div> :
...
const mapStateToProps = (state) => {
    const { activeConversationId, activeConversation, conversations } = state.Chat;
    const { userSidebar } = state.Layout;
    const { user } = state.User
    return { userSidebar, activeConversationId, activeConversation, user, conversations };
};

export default withRouter(connect(mapStateToProps, { openUserSidebar, setChatMessages })(UserChat));
