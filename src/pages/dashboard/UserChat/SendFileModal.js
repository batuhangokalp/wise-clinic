...
import { fetchConversationById, fetchMessagesByConversationId, sendMessage, setChatFile, uploadFile } from '../../../redux/actions';
...
export default function SendFileModal() {
    ...
    const handleSend = async () => {
        ...
        let url = await dispatch(uploadFile(formData))
        let request = {
        ...
        if (FileTypeId.Image?.includes(fileType)) {
            ...
            request["message_type_name"] = "document"
        }
    ...
    useEffect(() => {
        ...
        if (chatFile) {
            ...
            FileTypeId.Image?.includes(findFileType(chatFile?.type)) && console.log(URL.createObjectURL(chatFile))
