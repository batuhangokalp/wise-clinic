...
export const updateRole = (role) => {
    return async (dispatch) => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/roles/${role.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    "Accept": "*/*",
...
export const deleteRole = (roleId) => {
    return async (dispatch) => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/roles/${roleId}`, {
                method: 'DELETE',
                headers: {
                    "Accept": "*/*",
                    "Accept-Encoding": "gzip, deflate",
...
export const fetchPermissionsByRoleId = (roleId) => {
    return async (dispatch) => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/permissions/${roleId}`, {
                headers: {
                    "Accept": "*/*",
                    "Accept-Encoding": "gzip, deflate",
                    "User-Agent": "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:47.0) Gecko/20100101 Firefox/47.0",
